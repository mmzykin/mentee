...
def main_menu_keyboard(is_admin=False, has_assigned=False, can_spin=False, unread_announcements=0):
    ...
    keyboard.append(
        [InlineKeyboardButton(ann_text, callback_data="announcements:list")])

    # Meetings
    keyboard.append(
        [InlineKeyboardButton("📅 Мои встречи", callback_data="meetings:my")])
        ...
        for m in modules:
            ...
            if topics:
                for t in topics:
                    ...
                    text += (
                        f"  • <code>{t['topic_id']}</code>: {escape_html(t['name'])} ({count})\n"
                    )
            else:
                text += "  <i>(пусто)</i>\n"
            text += "\n"
